...
Without early validation, later modeling work risks getting blocked by tooling issues.
...
- Create and apply the initial migration (even if schema is minimal).
- Verify reset workflow.

Out of scope:

- Domain entity models beyond what is required to run a migration.
...
#### 3. Apply migrations (create DB and apply initial migration)
...
- Prisma resolves configuration and schema deterministically via `prisma.config.ts`.
- Reset is non-interactive (`--force`) and reapplies migrations cleanly.
