...
Status: Active. This document defines capability boundaries for the project.
Changes to milestone structure require architectural review and may require a new ADR.
...
Capability unlocked:
The project can be cloned, installed, migrated, and run consistently with governance in place.
...
Capability unlocked:
A draft MenuVersion workspace exists and can be retrieved through the domain layer.
...
Capability unlocked:
Categories can be created and ordered within the draft workspace.
...
Capability unlocked:
Items can be managed within draft categories with proper invariants.
...
Capability unlocked:
The menu can be safely published and consumed publicly.
...
Capability unlocked:
Administrative behavior is protected from unauthorized access.
